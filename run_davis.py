    ...
    for anno in ['1', '2']:
        ...
        with open(meta_file, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                if not line:
                    continue
                video, obj_id, exp = line.split(None, 2)
                exp = exp.strip().strip('"')
                if not os.path.isdir(os.path.join(img_folder, video)):
                    continue
                data.setdefault(video, []).append({'exp': exp, 'exp_id': obj_id})
        ...
        for idx_, video in enumerate(video_list):
            ...
            video_name = video
            metas = data[video]
            num_expressions = len(metas)
            frames = sorted(frame[:-4] for frame in os.listdir(os.path.join(img_folder, video_name)) if frame.endswith('.jpg'))
